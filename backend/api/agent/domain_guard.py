...
INJECTION_PATTERNS = [
    ...
    r'forget (the|your)?\s*(dataset|rules|instructions|system prompt)',
    r'act as (a|an)?\s*(general assistant|dan|jailbreak|unrestricted|hacker)',
    r'disregard (all)?\s*(instructions|rules|guidelines)',
    r'system prompt',
    r'reveal (the|your)?\s*(prompt|instructions|api key|secret|env|environment)',
    r'what (are|is) your (system prompt|hidden instructions|api key|secret)',
    r'show (me)?\s*(the)?\s*(api key|env|password|token|secret)',
    r'override (system|safety|security)',
    r'print (your)?\s*(prompt|secret|token|api_key)',
    r'expose (keys|credentials|variables)',
    r'who created you',
    r'tell me a (joke|poem|story|recipe)',
    r'write (a|an)?\s*(python script|poem|essay|code) for',
    r'capital of',
    r'weather in',
    r'who won the',
    r'translate to'
